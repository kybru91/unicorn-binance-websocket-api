...
# an explicit `events` query parameter — without it production delivers no
# payloads. UBWA subscribes to all event types documented in
# `BINANCE_FUTURES_USERDATA_EVENTS` by default; the `events` parameter on
# `create_stream()` lets the user narrow that down per stream.
#
...
if __name__ == "__main__":
    bdp = BinanceFuturesUserDataProcessor()
    try:
